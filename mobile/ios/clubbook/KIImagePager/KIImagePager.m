...
- (void) layoutSubviews
{
    ...
    if(blankImage)
    {
        [blankImage setFrame:_scrollView.bounds];
    }
}
#pragma mark - General
- (void) initialize
...
- (void) loadData
{
    ...
    if([aImageUrls count] > 0)
    {
        ...
        _pageControl.hidden = ([(NSArray *)[_dataSource arrayWithImages] count] > 1 ? NO : YES);
        ...
        if(self.overLayColor)
        {
            ...
            maskView.backgroundColor = self.overLayColor;
            [maskView setAutoresizingMask:self.autoresizingMask];
            [self addSubview:maskView];
            [maskView.superview bringSubviewToFront:maskView];
            
            UIView *lab = [self viewWithTag:10];
            if(lab)
            {
                [lab.superview bringSubviewToFront:lab];
            }
            lab = [self viewWithTag:9];
            if(lab)
            {
                [lab.superview bringSubviewToFront:lab];
            }
    ...
    {
        blankImage = [[UIImageView alloc] initWithFrame:_scrollView.bounds];
        [blankImage setImage:[_dataSource placeHolderImageForImagePager]];
        [_scrollView addSubview:blankImage];
        [_scrollView sendSubviewToBack:blankImage];
    }
}
...
- (void) initializePageControl
{
    ...
    _pageControl.frame = CGRectNull; //to hide it
...
{
    NSInteger currentPage = lround((float)scrollView.contentOffset.x / scrollView.frame.size.width);
...
- (void) slideshowTick:(NSTimer *)timer
{
    ...
    [_scrollView scrollRectToVisible:CGRectMake(self.frame.size.width * nextPage, _scrollView.contentOffset.y, self.frame.size.width, self.frame.size.width) animated:YES];
