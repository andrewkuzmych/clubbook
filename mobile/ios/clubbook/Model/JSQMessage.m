...
+ (instancetype)messageWithText:(NSString *)text sender:(NSString *)sender  type:(NSString *)type
{
    return [[JSQMessage alloc] initWithText:text sender:sender date:[NSDate date] type:type];
}
...
- (BOOL)isEqualToMessage:(JSQMessage *)aMessage
{
    return [self.text isEqualToString:aMessage.text]
            ...
            && [self.type isEqualToString:aMessage.type]
...
- (NSUInteger)hash
{
    return [self.text hash] ^ [self.sender hash] ^ [self.date hash] ^ [self.text hash];
}

- (NSString *)description
{
    return [NSString stringWithFormat:@"<%@>[ %@, %@, %@, %@ ]", [self class], self.sender, self.date, self.text, self.type];
}
