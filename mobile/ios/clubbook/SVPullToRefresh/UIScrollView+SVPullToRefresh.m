...
static CGFloat const SVPullToRefreshViewHeight = 20;
...
- (void)layoutSubviews
{
    ...
    else
    {
        ...
        CGRect textRect = [self.titleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth,self.titleLabel.font.lineHeight)
                                                          ...
                                                          attributes:@{NSFontAttributeName:self.titleLabel.font,NSParagraphStyleAttributeName: paragraphStyle.copy}
                                                             context:nil];
        
        titleSize = textRect.size;
        
        
        
        CGSize subtitleSize;
        paragraphStyle.lineBreakMode = self.subtitleLabel.lineBreakMode;
        textRect = [self.titleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth,self.titleLabel.font.lineHeight)
                                                      options:NSStringDrawingUsesLineFragmentOrigin
                                                   attributes:@{NSFontAttributeName:self.subtitleLabel.font,NSParagraphStyleAttributeName: paragraphStyle.copy}
                                                      context:nil];
        subtitleSize = textRect.size;
        ...
        if(subtitleSize.height > 0)
        {
            ...
            CGFloat titleY = minY;
            self.titleLabel.frame = CGRectIntegral( CGRectMake(labelX, titleY + 4, titleSize.width, titleSize.height) );
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset
{
    if(self.state != SVPullToRefreshStateLoading)
    {
        ...
        if(!self.scrollView.isDragging && self.state == SVPullToRefreshStateTriggered)
            ...
            if(contentOffset.y < scrollOffsetThreshold && self.scrollView.isDragging && self.state == SVPullToRefreshStateStopped && self.position == SVPullToRefreshPositionTop)
                ...
                if(contentOffset.y >= scrollOffsetThreshold && self.state != SVPullToRefreshStateStopped && self.position == SVPullToRefreshPositionTop)
                    ...
                    if(contentOffset.y > scrollOffsetThreshold && self.scrollView.isDragging && self.state == SVPullToRefreshStateStopped && self.position == SVPullToRefreshPositionBottom)
                        self.state = SVPullToRefreshStateTriggered;
                    else
                        if(contentOffset.y <= scrollOffsetThreshold && self.state != SVPullToRefreshStateStopped && self.position == SVPullToRefreshPositionBottom)
                            self.state = SVPullToRefreshStateStopped;
    }
    else
    {
...
- (SVPullToRefreshArrow*)arrow
{
    if(!_arrow)
    {
        if(!self.showArrow)
        {
            _arrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, 0, 0, 0)];
        }
        else
        {
